...
void main(void)
{
	...
	float jumpstart = 6.0;
	float swim = min(1.0, max(0.0, (iGlobalTime-128.0)*0.5 - 4.0));
	float slices = 3.0;
	float ind2 = floor(mod(uv.x*slices, slices));
	
	float rot = (iGlobalTime-128.0)*0.5 > rotationstart ? 1.0 : 0.0;
	float jump = (iGlobalTime-128.0)*0.5 > jumpstart ? 0.55 : 0.0;
	

	mat2 uvrot = makerot((iGlobalTime-128.0)*0.5 * rot);
	//uv += vec2(-0.5, -0.5);
	uv *= 1.0 + pow(sin((iGlobalTime-128.0)*0.5	*4.0*jump), 2.5);
	uv = uvrot * uv;
	...
	vec3 alter = vec3(ind < (iGlobalTime-128.0)*0.5);
	
	vec2 dir = vec2(cos(ind), sin(ind));
	
	float alpha = floor((iGlobalTime-128.0)*0.5 + ind*0.1);
	...
	dir = normalize(dir);
	float ofs = (iGlobalTime-128.0)*0.5;
