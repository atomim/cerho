...
void main() { 
	...
	for(float i=0.; i<1.0; i+=0.03){
		if(mod(length(m)-(iGlobalTime-i*0.077)*0.25,.3)<0.012 && mod(iGlobalTime*0.3+atan(m.x,m.y),3.74/10.0)>-3.14/60.0+3.14/20.0)
			c += vec4(0.14, 0.5, 0.0, 1.0);
		else if(mod(length(m)-(iGlobalTime-i*0.07)*0.5,.3)<0.012 && mod(atan(m.x,m.y),3.14/10.0)>-3.14/30.0+3.14/20.0)
			c += vec4(0.14, 0.5, 0.3, 1.0);
		else if(mod(length(m)-(iGlobalTime*0.76-i*0.037)*0.75,.1)<0.012 && mod(length(m)*1.4+atan(m.x,m.y),3.14/20.0)>-3.14/30.0+3.14/20.0)
	...
	if(uv.y<0.11)
		...
		col *= vec3(1.0)-texture2D(iChannel2, vec2(uv.x-mod((iGlobalTime+1.5)/32.0,1.0/32.0),0.119+mod(uv.y,0.14)+floor((iGlobalTime+1.5))*0.1)).rgb;
	else
		col *= vec3(0.0);
		col+=c.rgb;
