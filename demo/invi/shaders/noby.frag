			...
			{
				p *= (rotX(1.81*iGlobalTime*0.1)*rotY(1.46*iGlobalTime*0.1)*rotZ(1.667*iGlobalTime*0.1));
			...
			float boksi(in vec3 p, vec3 b, float r)
			{
				return length(max(abs(p)-b, 0.0)) - r;
			}

			float boksi2(vec3 p, vec3 b)
			{
				vec3 d = abs(p) - b;
			...
			{
				vec3 b1pos = vec3(r.x+0.2*sin(iGlobalTime*0.1*5.01), r.y-0.2*cos(iGlobalTime*0.1*4.91), r.z+0.2*cos(iGlobalTime*0.1*5.21));
				vec3 b2pos = vec3(r.x-0.2*sin(iGlobalTime*0.1*4.78), r.y+0.2*cos(iGlobalTime*0.1*5.38), r.z+0.2*sin(iGlobalTime*0.1*4.81));
				vec3 b3pos = vec3(r.x+0.2*cos(iGlobalTime*0.1*5.09), r.y-0.2*sin(iGlobalTime*0.1*5.33), r.z-0.2*cos(iGlobalTime*0.1*5.15));
				vec3 b4pos = vec3(r.x-0.2*cos(iGlobalTime*0.1*5.59), r.y-0.2*cos(iGlobalTime*0.1*4.49), r.z-0.2*sin(iGlobalTime*0.1*5.15));
				vec3 b5pos = vec3(r.x+0.2*sin(iGlobalTime*0.1*5.55), r.y+0.2*sin(iGlobalTime*0.1*4.63), r.z+0.2*cos(iGlobalTime*0.1*5.65));
			...
			void main(void)
			{
				...
				float zOff = -1.00+0.1*sin(iGlobalTime*0.1*3.0);

				// Rotation matrix & projection plane & camera
				mat3 rotation = rotX(2.31*iGlobalTime*0.1)*rotY(1.76*iGlobalTime*0.1)*rotZ(0.967*iGlobalTime*0.1);
				...
				float flicker = 0.99 + 0.02*smoothstep(0.0, 0.25, sin(2500.0*iGlobalTime*0.1));

				gl_FragColor = vec4( 0.02*noise(p*iGlobalTime*0.1)+f*flicker*vignette, 1.0);
			}
